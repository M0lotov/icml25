...
def projection_simplex(z):
    # Sort the input tensor in descending order
    z_sorted, _ = torch.sort(z, descending=True)
    
    # Compute cumulative sum of sorted vector and adjust by (1 - sum)
    cumulative_sum = torch.cumsum(z_sorted, dim=0)
    rho = (cumulative_sum - 1) / torch.arange(1, len(z) + 1, device=z.device, dtype=z.dtype)
    
    # Find the threshold theta
    mask = z_sorted > rho
    theta = rho[mask][-1]  # The last value in the valid range is the threshold
    
    z.copy_(torch.clamp(z - theta, min=0))
