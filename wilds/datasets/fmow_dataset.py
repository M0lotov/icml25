...
class FMoWDataset(WILDSDataset):
    ...
    def __init__(self, version=None, root_dir='data', download=False, split_scheme='official', seed=111, use_ood_val=True):
        ...
        elif self._split_scheme.startswith('time_before'):
            year = int(self._split_scheme.split('_')[2])
            year_dt = datetime.datetime(year, 1, 1, tzinfo=pytz.UTC)
            self.test_ood_mask = np.asarray(pd.to_datetime(self.metadata['timestamp'], format='ISO8601') <= year_dt)
            # use 3 years of the training set as validation
            year_plus_3_dt = datetime.datetime(year+3, 1, 1, tzinfo=pytz.UTC)
            self.val_ood_mask = np.asarray(pd.to_datetime(self.metadata['timestamp'], format='ISO8601') <= year_plus_3_dt) & ~self.test_ood_mask
            self.ood_mask = self.test_ood_mask | self.val_ood_mask
        elif self._split_scheme == 'time_mid':
            s_year_dt = datetime.datetime(2009, 1, 1, tzinfo=pytz.UTC)
            e_year_dt = datetime.datetime(2011, 1, 1, tzinfo=pytz.UTC)
            self.test_ood_mask = np.asarray((pd.to_datetime(self.metadata['timestamp'], format='ISO8601') >= s_year_dt) & (pd.to_datetime(self.metadata['timestamp'], format='ISO8601') < e_year_dt))
            # use 2 years of the training set as validation
            year_minus_1_dt = datetime.datetime(2008, 1, 1, tzinfo=pytz.UTC)
            year_plus_1_dt = datetime.datetime(2012, 1, 1, tzinfo=pytz.UTC)
            self.val_ood_mask = np.asarray((pd.to_datetime(self.metadata['timestamp'], format='ISO8601') >= year_minus_1_dt) & (pd.to_datetime(self.metadata['timestamp'], format='ISO8601') < year_plus_1_dt)) & ~self.test_ood_mask
            self.ood_mask = self.test_ood_mask | self.val_ood_mask
        else:
...
if __name__ == '__main__':
    FMoWDataset(root_dir='/usa/yanlin/workspace/data', split_scheme='official')